...
public class CoinSegment extends MapSegment {
	...
	public CoinSegment() {
		...
		if (ratio > 10) {
			m_image = BitmapManager.getInstance()
					.getBitmap(BitmapManager.Coin0);
			value = 10;
			m_coinType = Coin;
		} else if (ratio > 1) {
			m_image = BitmapManager.getInstance()
					.getBitmap(BitmapManager.Coin1);
			value = 100;
			m_coinType = Bag;
		} else {
			m_image = BitmapManager.getInstance()
					.getBitmap(BitmapManager.Coin2);
			value = 500;
			m_coinType = Gem;
		}
	...
	public void update(long elapsedTime) {
		if (!m_touchedPlayer) {
			...
			m_position
					.setY((float) (Math.sin(angle) / 20 * screenSize + screenSize / 2));
			}

			this.boundingBox.set((int) this.getM_position().getX(), (int) this
					.getM_position().getY(), (int) this.getM_position().getX()
					+ this.getWidth(),
